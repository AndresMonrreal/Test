...
var root = template(`<div style="display: none;"></div> <!>`, 1);

function ValidationMessage($$anchor, $$props) {
	...
	const messages = mutable_state();
	let level = prop($$props, "level", 8, 'error');
	let errorFor = prop($$props, "for", 8);
	let errors;
	let errorPath = mutable_state();
	let element = mutable_state();

	function getFormElement() {
		return get(element).closest('form');
	}
	...
	svelte.onMount(() => {
		setTimeout(() => {
			...
			set(errorPath, common.getPath(get(element), path));

			const formElement = getFormElement();

			if (level() === 'error') errors = errorStores[formElement.dataset.felteReporterSvelteId]; else errors = warningStores[formElement.dataset.felteReporterSvelteId];
		});
	});

	legacy_pre_effect(() => (get(errorPath), $errors()), () => {
		set(messages, get(errorPath) && common._get($errors(), get(errorPath)));
	});
	...
	if_block(
		node,
		() => !$$slots.placeholder || get(messages),
		($$anchor) => {
